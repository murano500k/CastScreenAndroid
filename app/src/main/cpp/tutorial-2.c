...
typedef struct _CustomData {
    ...
    GstElement *pipeline_out;   /* The running pipeline */
    GstElement *pipeline_in;   /* The running pipeline */
...
static pthread_t gst_app_thread_in;
static pthread_t gst_app_thread_out;
...
static void *app_function_in (void *userdata) {
    ...
    /* Set the pipeline to READY, so it can already accept a window handle, if we have one */
    gst_element_set_state(data->pipeline_in, GST_STATE_READY);

    data->video_sink = gst_bin_get_by_interface(GST_BIN(data->pipeline_in), GST_TYPE_VIDEO_OVERLAY);
    if (!data->video_sink) {
    ...
    /* Instruct the bus to emit signals for each received message, and connect to the interesting signals */
    bus = gst_element_get_bus (data->pipeline_in);
    bus_source = gst_bus_create_watch (bus);
    g_source_set_callback (bus_source, (GSourceFunc) gst_bus_async_signal_func, NULL, NULL);
    g_source_attach (bus_source, data->context);
    g_source_unref (bus_source);
    ...
    gst_object_unref (data->pipeline_in);
...
static void gst_native_finalize (JNIEnv* env, jobject thiz) {
  ...
  pthread_join (gst_app_thread_out, NULL);
  pthread_join (gst_app_thread_in, NULL);
